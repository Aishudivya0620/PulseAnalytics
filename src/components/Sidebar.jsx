...
const Sidebar = () => {
  ...
  return (
    <motion.aside 
      initial={{ width: sidebarCollapsed ? 88 : 280 }}
      animate={{ width: sidebarCollapsed ? 88 : 280 }}
      transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
      className="hidden md:flex flex-col h-screen fixed top-0 left-0 glass-panel border-r border-white/10 z-50 shadow-2xl"
    >
      {/* Collapse Toggle */}
      <button 
      ...
      {/* Logo Section */}
      <div className={`flex items-center gap-3 py-10 transition-all duration-500 ${sidebarCollapsed ? 'px-5' : 'px-8'}`}>
      ...
      <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto no-scrollbar">
        <div className={`transition-all duration-500 ${sidebarCollapsed ? 'px-2 flex justify-center' : 'px-5 mb-4'}`}>
        ...
        {navLinks.map((link) => {
          ...
          return (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) => 
                `nav-item transition-all duration-300 ${isActive ? 'active' : ''} ${sidebarCollapsed ? 'justify-center px-0' : 'px-5'}`
              }
            ...
            >
              ...
              {!sidebarCollapsed && link.name === 'Analytics' && (
      ...
      {/* Footer Section */}
      <div className={`p-6 transition-all duration-500 ${sidebarCollapsed ? 'px-2' : 'px-6'}`}>
        {!sidebarCollapsed ? (
          <motion.div 
        ...
        <button 
          ...
          className={`w-full flex items-center gap-3 py-3.5 rounded-2xl text-slate-400 dark:text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 transition-all duration-300 font-bold text-sm group ${sidebarCollapsed ? 'justify-center px-0' : 'px-4'}`}
